...
class RequestFactory{
    ...
    public static function createRequest(ConfigInterface $config)
    {
        ...
        if (true === is_object($class) && $class instanceof \Trafficbot\Request\RequestInterface) {
        ...
        if ($reflectionClass->implementsInterface('\\Trafficbot\\Request\\RequestInterface')) {          
            $request = new $class();
            $request->setConfig($config);
